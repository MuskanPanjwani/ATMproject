...
try{
    ...
    stmt2.executeUpdate("update cheque SET checkred = '1' where chequeid='"+ChequeID+"';");
    stmt3.executeUpdate("update bank_transaction SET balance  = balance - '"+cheque_amtcheck+"' where customerid='"+customeridcheck+"';");
    rs = stmt.executeQuery("select * from bank_transaction where customerid='"+customeridcheck+"';");
    if(rs.next()){
    	session.setAttribute("balance",rs.getString("balance"));
        String bal = (String)session.getAttribute("balance");
        stmt.setString(1,"Cheque wihdraw");
        stmt.setString(2,cheque_amtcheck);
        stmt.setString(3,bal);
        stmt.setString(4,customeridcheck);
